...
\begin{definition}
    Un grafo $G=(V,E)$ è costituito da un insisme di vertivi $V$ ed un insieme di archi $E$ ciascuno dei quali connette due vertici $V$ detti estremi dell'arco.\\
\end{definition}
\begin{definition}
...
\end{definition}
\begin{definition}
    Un \textbf{cappio} è un arco icui estremi coincidono\\
\end{definition}
\begin{definition}
...
una sequenza di $k+1$ vertici $\Pi= x_0, x_1, x_2,···, x_k$ tali che $x_0 = u, xk = v$ e $x_{i-1}x_i \in E$ per $i
= 1, . . . , k$.
...
Si espande uniformemente la frontiera tra i vertici e quelli non ancora scoeprti, 
in altre parole, scopre tutti i vertici a distanza $k$ da $s$ prima di scoprire quelli a distanza $k+1$\\
Per mantenere traccia del punto a cui si è arrivati nell'esecuzione dell'algoritmo, i vertici sono oclorati:
\begin{itemize}
...
\begin{proof}
    ...
    possiamo concludere dicendo che il passo base verifica la propriet`a del limite superiore.
...
\begin{enumerate}
    \item $v_r .d \le v1.d+ 1$ 
    \item $v_i .d \le v_{i+1}.d \text {per ogni }i in 1,2,...,r-1$
\end{enumerate}
\begin{proof}
...
\begin{proof}
Se $v$ non è raggiungibile da $s$ allora dalla proprietà dellimite superiore si ha che\\
$$v.d=d(s,v)=\infty$$\\
...
\end{enumerate}
\noindent Se consideriamo la sorgente banalmente l'inizializzazione avviene una sola volta quindi:
\begin{itemize}
    \item $s.color="GRIGI"O"$
    \item $s.d=0$
    \item $s.p=nil$
    \item $ENQUEUE(Q, S)$
\end{itemize}
