...
\end{verbatim}
\noindent FOTOOOOOOOOOOOOOOOOOOOOOO
\subsection{Analisi}
...
tempo. Successivamente chiama \textbf{$DFS_VISIT$} sui vertici $u$ che non sono stati ancora visitati.
All’interno di $DFS_VISIT$ viene incrementata la variabile tempo e viene assegnato al primo
marcatore di $u$, indicandone l’istante in cui viene colorato di $"GRIGIO"$. Dopodichè si prende
la lista di adiacenza del nodo $u$ e si scorre richiamando \textbf{ricorsivamente} $DFS_VISIT$ sul nodo $v$
...
\begin{proof}
    \textbf{Caso $u.f < v.d$}\\
    Sapendo che $u.d < u.f < v.f$ ne segeu che:\\
    $$ u.d < u.f < v.d < v.f $$\\
...
Sia $uv \in E$ un arco, qeusto può essere classificato in:
\begin{itemize}
    \item \textbf{Archi d'albero:} $v$ è scoperto di $u$ quindi ($v.status = UNEXPLORED$)
...
\end{itemize}
\noindent Il timestamp a cui fanno riferimento è quello di scoeprta dei nodi.
\colorbox{SALZO}{\parbox{\dimexpr\linewidth-2\fboxsep}{\begin{theorem}
...
\begin{itemize}
    ...
    \begin{figure}[H]
    \centering  
    \includegraphics[width=0.5\linewidth]{../images/ciclo_negativo.jpg}
    %\label{label}
\end{figure}
