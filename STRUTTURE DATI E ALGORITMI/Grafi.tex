...
\begin{definition}
    ...
    in due sottoinsiemi non vuoti $(S, V \ S)$
\end{definition}
\begin{definition}
    Un arco \textbf{interseca} il taglio $(S, V \ S)$ se uno dei suoi estremi è in S e l'altro in V/S
\end{definition}
\begin{definition}
    Un taglio (S, V\S) rispetta un isnieme di archi A se nessun arco in A interseca il taglio
\end{definition}
\begin{definition}
...
(S, V\S) un taglio qualsiasi di $G$ che rispetta $A$ e sia $a= (u, v)$ un arco leggero per il taglio
(S, V\S). Allora l’arco a è sicuro.
\end{theorem}}}
\begin{proof}
...
\begin{figure}[H]
    \centering  
    \includegraphics[width=0.9\linewidth]{../images/analisi_kruskal.png}
\end{figure}
...
\end{itemize}
Il costo dell’algoritmo di Kruskal è $O(n^2)$, il costo dipende principalmente dall’andamento della
funzione \textbf{Union}. Quindi per migliorare il costo di questo algoritmo è necessario modificare
...
\begin{figure}[H]
    \centering  
    \includegraphics[width=0.9\linewidth]{../images/analisi_kruskal2.png}
\end{figure}
...
\end{figure}
Innanzitutto si sceglie un nodo da cui partire che fungerà da sorgente, in questo caso il nodo $a$.
