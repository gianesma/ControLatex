...
basta implementare un algoritmo che verifica che $m = n-1$, ovvero effettuare un confronto
...
Inoltre l'ordine può essere necessario per la visita pre-ordine, in-ordine e post-ordine.\medskip\\
...
\begin{figure}[H]
    \centering  
    \includegraphics[width=0.9\linewidth]{../images/albero_binario.png}
\end{figure}
...
\begin{figure}[H]
    \centering  
    \includegraphics[width=0.9\linewidth]{../images/alberi_bin_ricerca.png}
\end{figure}
...
\begin{enumerate}
    ...
    Ciò implica che la radice venga visitata dopo dei suoi sottoalberi. 
\end{enumerate}
...
\end{theorem}}}
\begin{proof}
Sia $T(n)$ il tempo richiesto da Inorder(x) per visitare un sottoalbero di $n$ nodi con radice $x$, è necessario dimostrare che $T(n)$ è $\Omega(n)$ e allo stesso tempo $O(n)$.\\
Dire che $T(n)$ è $\Omega(n)$ significa che $T(n)$ è almeno pari a $c \cdot n$ per qualche costante $c$ e per $n$ 
sufficientemente grande. Durante l'attraversamento in-ordine dobbiamo visitare tutti i nodi, ogni nodo deve essere 
...
\end{enumerate}
\noindent Supponiamo che $\mathbf{n=0}$, per visitare un nodo serve una piccola quantità di tempo, basta verificare che $x=nil$, ecco perchè $T(n=0)=c$.\\
Supponendo che $\mathbf{n>0}$ e che il sottoalbero sinistro abbia k nodi e il sottoalbero destro ne abbia $n-k-1$, il tempo totale $T(n)$ è quindi minore o uguale alla somma 
...
$$T(n)=(c+d)n+c \text{ per ogni } n \ge 0$$\\
...
\begin{enumerate}
    ...
    \item \textbf{Minimum(x) o Maximum(x):} dato un albero con radice $x$, restituisce l'elemento la cui chiave è minima o massima
...
\begin{enumerate}
    \item \textbf{Insert(T,z):} l'idea è quella di inserire nell'albero un nuovo nodo z sottoforma di foglia. Affinchè ciò 
    ...
    \begin{figure}[H]
    \centering  
    \includegraphics[width=0.9\linewidth]{../images/alberi_inserimento.png}
\end{figure}
    ...
    \begin{itemize}
        ...
        \begin{figure}[H]
            \centering  
            \includegraphics[width=0.45\linewidth]{../images/delete1_alberi.png}
        \end{figure}
        ...
        \begin{figure}[H]
            \centering  
            \includegraphics[width=0.45\linewidth]{../images/delete2_alberi.png}
        \end{figure}
...
sostituire z
    ...
    \begin{figure}[H]
            \centering  
            \includegraphics[width=0.45\linewidth]{../images/delete3_alberi.png}
        \end{figure}
\end{enumerate}
\noindent \textbf{Pseudocodice}
\begin{verbatim}
...
cammino semplice che inizia dal nodo $x$ (ma non lo include) e termina in una foglia.
...
\texttt{il sottoalbero radicato in un nodo x qualunque contiene almeno $2^{bh(x)}-1$ nodi interni}\\
\textbf{Passo base:}\\
$h=0$ (x è una foglia)\\
...
\begin{figure}[H]
    \centering  
    \includegraphics[width=0.4\linewidth]{../images/dim_radici_nere1.png}
\end{figure}
\noindent Per l'ipotesi induttiva, poichè l'altezza dei figli di x è inferiore all'altezza $h$ di x, possiamo affermare 
che ciascun figlio ha almeno $2^{bh(x)-1}-1$ nodi interni.
\begin{figure}[H]
    \centering  
    \includegraphics[width=0.4\linewidth]{../images/dim_radici_nere2.png}
\end{figure}
\noindent Quindi, il sottoalbero radicato in x ha almeno $(2^{bh(x)-1}-1)+(2^{bh(x)-1}-1)+1=2^{bh(x)}-1$ nodi interni da cui segue la proprietà
\begin{figure}[H]
    \centering  
    \includegraphics[width=0.4\linewidth]{../images/dim_radici_nere3.png}
\end{figure}
\noindent Per concludere la dimostrazione, ragioniamo ora sull'altezza dell'albero $h$
\begin{itemize}
    \item per la proprietà 4 degli alberi rosso-neri, almeno la metà dei nodi su un cammino verso le fglie sono neri 
    \item ne segue che $bh(root) \ge h/2$, e quindi
    \item $n \ge 2^{h/2}-1$
    \item $(n+1) \ge 2^{h/2}$
    \item $\log (n+1) \ge h/2$
    \item $ h \le 2\log (n+1) $
\end{itemize}
\end{proof}
\noindent Un'immediata conseguenza di questa definzione è che su di un albero rosso-nero con $n$ nodi interni le 
operazioni Search, Minimum, Maximum, Successor e Predecessor richiedono 
tutte un tempo $O(\log n)$.\\
\begin{definition}
    Negli alberi rosso-neri le operazioni \textbf{Insert} e \textbf{Delete} sono opportunamente modificate inserendo una fare 
    di ribilanciamento per garantire un'altezza dell'albero\\
    $$\mathbf{h=O(\log n)}$$
\end{definition}