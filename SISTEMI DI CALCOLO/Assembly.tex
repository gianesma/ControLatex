...
Il padding sarebbe memoria vuota per garantire l'allineamento corretto dei campi.
    \item \textbf{Passaggio del parametro:} $\rightarrow$
    il valore viene inserito nello stack: (riprendendo l'esempio di sopra)
    \begin{verbatim}
pushl $8
    \end{verbatim}
...
list_add_first:
    ...
    # p = *l
    movl 8(%ebp), %eax     # eax = l
    movl (%eax), %ecx      # ecx = p

    # malloc(sizeof(node_t)) = 8 byte
    pushl $8        #passo sizeof(node_t) come parametro
    call malloc     #chiama malloc
    addl $4, %esp   #pulisco lo stack 
                    #poichè grandezza 
                    #del parametro è 4 byte
    ...
    # n->elem = elem
    movw 12(%ebp), %dx  # dx = elem
    movw %dx, (%eax)    # scrivo elem in n->elem (offset 0)

    # n->next = p
    movl %ecx, 4(%eax) 

    # *l = n
    movl 8(%ebp), %edx  # edx = l
    movl %eax, (%edx)   # scrivo n in *l
...
END:
    movl %ebp, %esp  #ripristino dello stack
    popl %ebp
    ret
