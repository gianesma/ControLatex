...
\section{Allineamento in Stack}

Mettemo sto disegno o no? nonnooooo\\
\begin{minipage}{0.4\textwidth}
    \begin{verbatim}
void g(char *c);
void f(){
...
f.s
    \end{verbatim}
\end{minipage}\medskip\\
\textbf{\textcolor{red}{Importante:}} in qualsiasi momento dell'esecuzione di qualsiasi funzione, 
lo stack pointer deve contenere un indirizzo multiplo di 4.\\
Attenzione il compilatore tipicamente allinea lo stack a 16 byte.\\
Altro disegno altra corsa\medskip\\
\begin{minipage}{0.3\textwidth}
    \begin{verbatim}
char a,b,c
g(&a, &b, &c);
...
\begin{minipage}{0.6\textwidth}
    \begin{verbatim}
#Prologo
subl $16, %esp  #spazio per a,b,c e per i parametri di g
                #dove i char occupano 1 byte ciascuno, 
                #ma per allineamento ne occupano 4
                #quindi 3 char occupano 3 byte, 
                #più i 4 byte per il ogni parametro di g fanno 16
                #ovvero il multiplo di 4 
                #più vicino a 3 + 3*4 = 15 
...

f.s
    \end{verbatim}
\end{minipage}\medskip\\
foto di un disegno con stack allineato a 16 byte\\
\textbf{\textcolor{red}{Importante:}} i dati di dimensione x nella stack
sono allineati ad indirizzi multipli di x.\medskip\\
\begin{minipage}{0.4\textwidth}
    \begin{verbatim}
char[5] a;
short b;
...
\begin{minipage}{0.5\textwidth}
    \begin{verbatim}
#Prologo
subl $16, %esp  #spazio per a,b e per i parametri di g
                #dove a occupa 5 byte ma per allineamento 
                #ne occupa 8
                #b occupa 2 byte ma per allineamento ne occupa 4
                #quindi 8 + 4 + 4*2 = 16
...
...
f.s
    \end{verbatim}
\end{minipage}\medskip\\
\subsection{L'allineamento in memoria di struct }
Per tradurre una struct in Assembly, dobbiamo partire col farci
queste domande:
\begin{enumerate}
    \item Dove inizia in memoria ciascun campo della struct?
    \item quanto spazio occupa complessivamente la struct
\end{enumerate}
L'allineamento in memoria della struct segue 4 regole:
\begin{enumerate}
    \item Un dato di dimensione x deve trovarsi allineato in
    memoria ad un indirizzo multiplo di x
    \item La struct in memoria ha un allineamento complessivo
    che deriva dal campo di tipo più grande
    \item La dimensione complessiva della struct in memoria è
    multipla della dimensione del suo campo più grande
    \item 
\end{enumerate}
\begin{minipage}{0.3\textwidth}
    \begin{verbatim}
...
struct s{   // base
    ...
    int y;  // +4  |x x x x|
            // sizeof s = 8;
            // di cui 1 byte solo è sprecato
            // redendo più efficiente 
            // l'uso della memoria
}
    \end{verbatim}
\end{minipage}\medskip\\
Vediamo un esempio con traduzione ora in Assembly: \medskip\\
\begin{minipage}{0.3\textwidth}
    \begin{verbatim}
struct s{   // base
    char x; // +0
